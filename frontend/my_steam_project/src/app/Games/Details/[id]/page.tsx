import { getDetailById } from '@/app/api/game/gameActions'
import Head from 'next/head'
// import Image from 'next/image'
import React from 'react'
import { Button, Image } from '@nextui-org/react'
import AddButton from '@/app/Basket/Actions/AddButton'
...
export default async function page({params}:{params:{id:string}}) {
  ...
  <div className='border border-gray-500 rounded-sm' >
    ...
    <div className="lg:w-4/5 mx-auto flex flex-wrap row-auto">
      <div className="col lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0 flex" id='deneme1'>
        <video src={data.videoUrl} controls className="flex-grow"></video>
      </div>
      <div className="col lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0" id='deneme2' >
        <h2 className="text-sm title-font text-gray-500 tracking-widest"> {data.gameName} </h2>
        <h1 className="text-gray-900 text-3xl title-font font-medium mb-1"> {data.gameAuthor} </h1>
        ...
        <p className="leading-relaxed"> {data.gameDescription} </p>
        <div className="flex">
